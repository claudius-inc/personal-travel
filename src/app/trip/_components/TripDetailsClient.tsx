...
export default function TripDetailsClient({
  ...
  const itemsByDay = trip.itineraryItems.reduce(
    (acc: Record<string, Item[]>, item: Item) => {
      // handle either string dayIndex from SQLite or number dayIndex
  ...
  return (
    <main className="container mx-auto max-w-5xl py-10 px-4 md:px-6">
      ...
      <Tabs defaultValue="itinerary" className="w-full">
        <TabsList className="grid w-full max-w-md grid-cols-3 bg-neutral-100 border border-neutral-200 rounded-lg p-1 mb-8">
          <TabsTrigger
            value="itinerary"
            className="rounded-md data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm text-neutral-600"
          >
        ...
        >
          {/* Upload Section */}
          <Card className="bg-white border-neutral-200 border-dashed shadow-sm">
            <CardHeader>
            ...
            <CardContent className="flex items-center gap-4">
              <Input
                ...
                className="bg-neutral-50 border-neutral-200 max-w-xs text-neutral-900"
              />
              <Button
                  ...
                  <div key={day} className="relative">
                    ...
                    <div className="space-y-4 pl-4 border-l-2 border-neutral-200 ml-4">
                      {itemsByDay[day].map((item: Item) => (
                        <Card
                          key={item.id}
                          className="bg-white border-neutral-200 shadow-sm relative -left-4 w-[calc(100%+16px)]"
                        >
                          <CardHeader className="py-4">
                            <div className="flex justify-between items-start">
                              ...
                              <Button
                                variant="outline"
                                size="sm"
                                className="border-indigo-200 text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700 bg-white"
                          ...
                          {/* Insights Dropdown */}
                          {insights[item.id] && (
                            <CardContent className="bg-indigo-50/50 rounded-b-xl border-t border-indigo-100 py-4 mt-2">
        ...
        >
          <Card className="bg-white border-neutral-200 overflow-hidden relative shadow-sm">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-2 space-y-4">
              <div className="flex justify-between items-center bg-white border border-neutral-200 p-6 rounded-2xl shadow-sm">
                <div>
                  <p className="text-neutral-500 text-sm">Total Spent</p>
                  <h3 className="text-4xl font-bold mt-1 text-neutral-900">
                    ${totalExpense.toFixed(2)}
                  </h3>
                </div>
                <div className="text-right">
                  <p className="text-neutral-500 text-sm">Remaining</p>
                  <h3
              ...
              <div className="space-y-3 mt-8">
                <h4 className="font-medium text-lg mb-4 text-neutral-900">
                  Recent Transactions
                </h4>
                {trip.expenses.length === 0 ? (
                  ...
                  trip.expenses.map((exp: Expense) => (
                    <div
                      key={exp.id}
                      className="flex justify-between items-center p-4 rounded-xl bg-white border border-neutral-200 hover:border-neutral-300 transition shadow-sm"
                    >
                      <div>
                  ...
                  <form onSubmit={handleAddExpense} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className="bg-neutral-50 border-neutral-200 text-neutral-900 [color-scheme:light]"
                      />
                    </div>
                    <Button
                      type="submit"
                      disabled={addingExpense}
                      className="w-full bg-indigo-600 hover:bg-indigo-700 mt-4 shadow-sm"
                    >
                      {addingExpense ? (
                        <Loader2 className="w-4 h-4 animate-spin mr-2" />
                      ) : (
